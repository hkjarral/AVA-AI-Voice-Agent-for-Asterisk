...
const preview = {
  ...
  analysis: {
    ...
    event_count: 3,
    findings: [{ severity: 'warning', message: 'Provider jitter', component: 'src.providers.google_live' }],
    lifecycle: [{ name: 'Call started', captured: true }, { name: 'Call ended', captured: true }],
    missing_evidence: [],
  ...
  sources: {
    ...
    local_ai_server: { selected: true, required: false, recommended: false, reason: 'Probably not needed.' },
    admin_ui: { selected: true, required: false, recommended: false, reason: 'Optional management context.' },
  },
};
...
describe('CallTroubleshootView', () => {
  ...
  it('shows actionable summary and defaults optional evidence to selected', async () => {
    render(
      <MemoryRouter>
        <CallTroubleshootView callId="1789247215.144" events={[]} onChooseAnotherCall={vi.fn()} />
      </MemoryRouter>,
    );

    expect(await screen.findByRole('heading', { name: 'Troubleshoot Call' })).toBeInTheDocument();
    ...
    expect(screen.getByText(/No recordings, phone numbers, API keys or secrets/)).toBeInTheDocument();
  });
