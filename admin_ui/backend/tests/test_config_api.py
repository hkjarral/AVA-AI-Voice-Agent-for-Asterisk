...
def _agents_db_with_rows(tmp_path, monkeypatch, rows):
    db_path = tmp_path / "agents.db"
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            "CREATE TABLE agents (slug TEXT, display_name TEXT, audio_profile TEXT)"
...
def test_profile_usage_guard_confirmed_override_allows_in_use_edit(
    tmp_path, monkeypatch
):
    _agents_db_with_rows(
        tmp_path, monkeypatch, [("ava-demo", "Ava Demo", "telephony_ulaw_8k")]
    )
    old = {
        "profiles": {
            "default": "telephony_ulaw_8k",
    ...
    config._assert_in_use_audio_profiles_unchanged(
        old, new, allow_in_use_changes=True
    )


def test_profile_usage_guard_confirmed_override_still_blocks_deletion(
    tmp_path, monkeypatch
):
    _agents_db_with_rows(
        tmp_path, monkeypatch, [("ava-demo", "Ava Demo", "telephony_enhanced_8k")]
    )
    old = {
        "profiles": {
            "default": "telephony_ulaw_8k",
    ...
    assert exc_info.value.status_code == 409
    assert "Ava Demo" in str(exc_info.value.detail)
    assert "Move those Agents to another profile" in str(exc_info.value.detail)


def test_profile_usage_guard_confirmed_override_still_blocks_default_repoint(
    tmp_path, monkeypatch
):
    _agents_db_with_rows(
        tmp_path, monkeypatch, [("default-agent", "Default Agent", None)]
    )
    profiles = {
        "telephony_ulaw_8k": {"transport_out": {"encoding": "ulaw"}},
        "telephony_enhanced_8k": {"transport_out": {"encoding": "ulaw"}},
...
def test_profile_usage_guard_confirmed_override_skips_agent_store_read(
    tmp_path, monkeypatch
):
    # A pure in-use body edit under the confirmed override never needs the
    # Agent store, so an unreadable store must not block it (contrast with
    # test_profile_usage_guard_fails_closed_when_agent_store_is_invalid).
    db_path = tmp_path / "agents.db"
