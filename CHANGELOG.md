...
- **The default Audio Profile owns the ExternalMedia RTP codec:** the RTP wire codec is now derived from `profiles.<default>.transport_out` when the engine starts (ExternalMedia runs one process-wide codec), and the per-call ExternalMedia channel is created with the running RTP server's codec so the two can no longer disagree. The Audio Transport page drops the Codec, Direction, Internal Format and Sample Rate fields — the codec is edited on Audio Profiles, the engine-side resample format is managed automatically, and the legacy `external_media.codec`/`format`/`sample_rate`/`direction` YAML keys remain honored as fallbacks for configs predating Audio Profiles. The audio-alignment view resolves the codec through the same shared helper, warns per Agent when a profile disagrees with the process-wide codec, and notes when a legacy YAML codec is being superseded. The profile dialog's optional Transport Input section now states plainly that it is advisory on AudioSocket (per-frame format header governs) and acts as the declared inbound expectation for RTP decode fallbacks and diagnostics.
