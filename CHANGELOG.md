...
- **`AGENTS_DB_PATH` is now honored by both agent stores** (`src/core/agent_store.py`, `admin_ui/backend/agents_store.py`): the env var was read only when building the support-bundle snapshot — neither the engine reader (`EngineAgentStore`, constructed with no arg by the orchestrator) nor the admin write store (`AgentsStore`) honored it, so setting it relocated nothing. Both constructors now default to `os.getenv("AGENTS_DB_PATH", DB_DEFAULT)`. When the env is unset the historical default (`/app/data/operator/agents.db`) is preserved exactly — no behavior change for existing installs (MED-C2).
