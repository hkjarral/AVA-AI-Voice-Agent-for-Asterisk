...
- **AudioSocket call lifecycle stabilization across full agents and pipelines** (`src/engine.py`, `src/core/streaming_playback_manager.py`, provider/pipeline adapters): provider output, pipeline producer ownership, talk-detect echo suppression, terminal fallback, and caller-inactivity grace now converge on the caller-facing transport drain instead of racing provider generation or duplicate input-end events. The supervised matrix covers every configured full-agent provider and all three modular pipelines; revision-pinned evidence is recorded in `docs/baselines/golden/v7.3.2-validation-matrix.md`.
...
- **Local pipeline readiness now recovers after startup races** (`src/engine.py`, `src/pipelines/orchestrator.py`): valid pipelines using Local STT/LLM/TTS components retry failed startup connectivity checks in a targeted background loop until Local AI Server is reachable, so `/health` and Admin live status no longer stay permanently degraded after the server finishes loading. Invalid pipelines remain unavailable, and cloud-only pipelines are not background-polled.
- **Updater rollback and validation hardening** (`updater/run.sh`, `updater/Dockerfile`, CLI update checks): updater containers now drop from root to the host checkout owner while retaining Docker-socket group access, preventing branch switches and fetch metadata from making the repository root-owned. The rollback active-call probe keeps stdin attached so it returns a real count, nested runtime databases no longer create false dirty-worktree stashes, updater-built CLIs report their source ref instead of the stale development fallback, and post-update validation retries a transient ARI-registration warning once after services settle.
