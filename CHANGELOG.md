...
- **Per-agent email fields (`email_recipient`, `email_from`, `email_enabled`) now honored on the YAML / DB-fallback path (#437)** (`src/core/transport_orchestrator.py`): `_load_contexts()` and `_yaml_context_config()` silently dropped the three per-agent email override keys added in v7.0.1 — they were parsed by `EngineAgentStore.resolve()` from `agents.db` but never read from the YAML context dict, so headless (YAML-only) installs and the `AgentStoreReadError` fallback path always returned `None` for all three fields, causing post-call summaries to fall back to the global/per-context settings rather than the per-agent overrides. Both functions now read `email_recipient`, `email_from`, and `email_enabled` from the YAML context dict via `.get()`, preserving the tri-state semantics: an absent key stays `None` (inherit), and `False` is never coerced to `None`.
