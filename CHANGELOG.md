...
- **Opt-in alias-safe telephony downsampling and audio-policy controls** ([#553](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/553), providers, modular pipelines, Admin UI): OpenAI Realtime, Google Live, ElevenLabs Agent, Grok, and shipped modular TTS adapters can select a stateful band-limited 16/24 kHz-to-8 kHz converter that removes out-of-band energy before G.711 playback. The compatibility linear converter remains the default and each full-agent provider has an independent environment rollback override. Provider and pipeline forms expose the effective conversion, pipeline TTS overlap can be overridden without changing other pipelines, Local Whisper segmentation can be tuned per pipeline/session, and invalid resampler, segmentation, overlap, or fixed encoding/rate combinations fail config validation before apply.
