...
- **Per-extension custom device-state editor** ([#577](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/577)): each Live Agent's advanced routing settings now include an "Availability Signals" editor for mapping custom Asterisk device states (e.g. `Custom:DND102`) to a Busy/Do Not Disturb/Away/Unavailable status, alongside the existing automatic ARI-based detection.
- **Global device-state value mapping editor** ([#577](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/577)): Tools → Check Extension Status now exposes a collapsed "State Value Mapping" panel to customize which raw Asterisk device-state values count as Free, Busy, or Not available, with a "Reset to defaults" action. Values not explicitly listed as Free or Busy are treated as not available (fail-closed), matching the existing backend default.
