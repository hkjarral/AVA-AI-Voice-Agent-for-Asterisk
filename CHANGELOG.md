...
- **PyTorch backends use a pinned, platform-correct dependency pair** (`local_ai_server/requirements.txt`, `local_ai_server/Dockerfile`, `local_ai_server/Dockerfile.gpu`): Kokoro, MeloTTS, and Silero share pinned PyTorch/Torchaudio versions. CPU builds explicitly use PyTorch's CPU wheel index instead of silently downloading multi-gigabyte CUDA libraries, while the NVIDIA image retains CUDA wheels.
