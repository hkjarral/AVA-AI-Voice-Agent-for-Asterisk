...
- **Agent-tool review hardening closes migration, export, editor, reload, and scope edge cases** (`admin_ui/backend`, `admin_ui/frontend`, `src/tools`, `src/engine.py`): starter provisioning and normal Agent writes now defer whenever legacy YAML Contexts are still waiting for the atomic engine import, preventing an Admin-first startup from shadowing old dialplan targets, and starter routing chooses an explicitly configured full-agent default before a stale active pipeline. Starter prompts restore safe defaults after trimming blank setup input, and default selection is covered by the same rollback boundary as creation. Pipeline starter Agents retain a compatible full-agent transport provider when one is available (including `local` for local-hybrid pipelines), so their per-Agent audio profiles resolve before pipeline startup. A completed Context migration remains authoritative after its final Agent is deleted, and pre-populated stores now record that completion before returning, so retained legacy YAML cannot resurrect removed Agents on restart. Engine-owned imports persist the same normalized Context digest used by Admin drift detection, and early v7.4 null markers backfill that digest without replacing an existing baseline, preventing an immediate false-positive reconcile against unchanged YAML. Legacy resource-policy promotion runs once at Admin UI startup instead of scanning Agent rows on every request, YAML export cannot let stale `extra_json` overwrite canonical tool policies, and migration emits canonical compact JSON. Agent editors clear failed inventory loads, preserve invalid saved policies for server-side rejection instead of silently broadening them to global inheritance, and provide controls to remove stale resource keys. Tool generation construction moves off the health-server event loop while orchestrator publication remains atomic, and per-call pipeline adapters build schemas from the same captured registry they execute against. Calendar tools share one Agent-scope resolver that cannot replace an intentionally empty immutable snapshot with live YAML, explicit Google Calendar denial blocks the legacy single-calendar path, and voicemail guidance uses its public mailbox resolver. Narrowed transfer policies now also force configured-only extension-status checks and reject placeholder/non-object destinations as stale, preventing a permissive global setting or incomplete inventory entry from exposing unusable status/transfer targets.
- **Agent routing and legacy migration now fail closed at two review boundaries** (`src/engine.py`, `src/core/legacy_agent_migration.py`): an explicit unknown or inactive `AI_AGENT`/`AI_CONTEXT` can no longer continue under the default provider, prompt, or tools; the call records a context-resolution failure before runtime/provider setup. When the one-time Context importer replaces an existing empty WAL-mode `agents.db`, it now requires a successful WAL checkpoint and removes the old `-wal`/`-shm` sidecars before publishing the migrated database, preventing stale frames from being replayed against the replacement file.
