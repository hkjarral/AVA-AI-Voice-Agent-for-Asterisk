...
- **Editable provider audio formats with out-of-config defaults:** a new `GET /api/config/providers/audio/baselines` endpoint exposes the canonical provider audio baselines, and a shared provider-form section renders them as editable encoding/sample-rate fields. Values equal to the shipped default stay out of `ai-agent.yaml`; only real overrides are stored, and each override is labeled with the default it replaces. The ElevenLabs Agent form adopts the section (Asterisk input, provider-native input, provider output, and Asterisk output pairs, including A-law).
