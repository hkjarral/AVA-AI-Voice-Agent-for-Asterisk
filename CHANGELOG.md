...
- **Agent-tool review hardening closes migration, export, editor, and reload edge cases** (`admin_ui/backend`, `admin_ui/frontend`, `src/tools`, `src/engine.py`): starter provisioning now defers whenever legacy YAML Contexts are still waiting for the atomic engine import and chooses an explicitly configured full-agent default before a stale active pipeline. Legacy resource-policy promotion runs once at Admin UI startup instead of scanning Agent rows on every request, YAML export cannot let stale `extra_json` overwrite canonical tool policies, and migration emits canonical compact JSON. Agent editors clear failed inventory loads, preserve invalid saved policies for server-side rejection instead of silently broadening them to global inheritance, and provide controls to remove stale resource keys. Tool generation construction moves off the health-server event loop while orchestrator publication remains atomic; calendar tools share one Agent-scope resolver that cannot replace an intentionally empty immutable snapshot with live YAML, explicit Google Calendar denial blocks the legacy single-calendar path, and voicemail guidance uses its public mailbox resolver.
