...
- **Configurable activity handling for Google Live** ([#637](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/637)): the Google Live provider accepts an `activity_handling` key that maps to `realtimeInputConfig.activityHandling` in the session setup message, the sibling of the `automaticActivityDetection` block the existing `vad_*` keys already populate. Setting it to `NO_INTERRUPTION` makes the model finish its current response instead of cancelling generation the moment the caller makes a sound, which is what disclosures that must be read in full, dosage and safety instructions, and spoken confirmation or case numbers need. The key governs Gemini only. AVA's local barge-in runs on its own path and flushes playback directly, and `google_live` is already in the default `barge_in.provider_fallback_providers`, so on a stock configuration the caller still hears the agent cut at `barge_in.min_ms` (250 ms) while `barge_in.provider_output_suppress_ms` (1200 ms) discards the model's continuing audio. Making the change audible requires the companion `barge_in` settings documented in the PR. It suits readouts that must complete rather than dialogue: an agent that cannot be interrupted reads as a machine, and callers speaking across its output leave only sentence tails to transcribe. `START_OF_ACTIVITY_INTERRUPTS` is accepted so the API default can be stated explicitly; `ACTIVITY_HANDLING_UNSPECIFIED` is rejected because sending it is equivalent to omitting the key. Validation sits in the provider rather than the config model and is fail-safe: an unrecognized value omits the key for that one provider instance instead of failing config load for every agent in the file, which matters because Google Live closes the websocket with code 1007 on an unknown enum value. The field defaults to `None`, `None` omits the key, and an omitted key leaves the API's own behavior in force, so any deployment that does not set `activity_handling` sends exactly the setup message it sends today.
