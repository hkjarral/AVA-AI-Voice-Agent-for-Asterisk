...
- **Optional Local AI backends use a compatible, platform-correct dependency set** (`local_ai_server/requirements.txt`, `local_ai_server/Dockerfile`, `local_ai_server/Dockerfile.gpu`): Kokoro, MeloTTS, and Silero share pinned PyTorch/Torchaudio and Hugging Face versions. CPU builds explicitly use PyTorch's CPU wheel index instead of silently downloading multi-gigabyte CUDA libraries, while the NVIDIA image retains CUDA wheels. Every final dependency layer runs `pip check`, so incompatible optional-backend combinations now fail the image build.
