...
- **Local full-agent barge-in reacts to normalized caller audio and abandons the interrupted exchange** (`src/engine.py`, `src/providers/local.py`, `local_ai_server/ws_protocol.py`, `local_ai_server/server.py`): sustained caller energy on isolated Local AI media no longer requires a second, conflicting enhanced-VAD vote. AI Engine sends the cancellation control before playback cleanup, immediately clearing Whisper suppression, and tells Local AI Server to remove only the active user/assistant pair from weak-model history so the replacement turn cannot resume the abandoned answer. Hosted-provider fallback thresholds and history remain unchanged.
