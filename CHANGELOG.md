...
- **Updater rollback fails closed when active-call state is unknown:** Rollback service changes now stop when the AI Engine active-call probe fails or returns an invalid count. Operators must restore the probe and retry, or explicitly set `AAVA_UPDATE_FORCE_ACTIVE_CALLS=true` as an emergency override that may interrupt calls.
