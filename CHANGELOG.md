...
- **Config-cache rollout — batch 4, completes the rollout (UX-01)** (`admin_ui/frontend/src/pages/ContextsPage.tsx`, `admin_ui/frontend/src/pages/ProfilesPage.tsx`, `admin_ui/frontend/src/pages/Advanced/RawYamlPage.tsx`, `admin_ui/frontend/src/pages/Advanced/LLMPage.tsx`): the remaining config pages now seed (cache-first) from the shared `/api/config/yaml` cache, so navigating to them no longer flashes a loading state. ContextsPage preserves its post-load `fetchMcpTools`/`fetchToolCatalog` side-effects and force-refetches after saves; LLMPage swaps the config entry of its `Promise.allSettled` for the cache; RawYamlPage seeds its raw-YAML editor from the cached `content`. With this, all config pages that read `/api/config/yaml` share the cache.
