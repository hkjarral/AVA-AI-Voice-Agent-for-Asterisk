...
- **Whisper Large v3 Turbo STT model init failure (#297)**: `FASTER_WHISPER_STT_MODELS` listed `model_path: "large-v3-turbo"` but `faster-whisper` 1.0.3 (our pinned version) does not recognize `"turbo"` or `"large-v3-turbo"` as valid model size shorthands — turbo support was only added in `faster-whisper` 1.1.0. Fix: changed `model_path` to the HuggingFace repo ID `"Systran/faster-whisper-large-v3-turbo"`, which `faster-whisper` 1.0.3 accepts directly and downloads the official CTranslate2-converted turbo model. Also added `download_root` to persist downloaded models under the bind-mounted volume (`/app/models/stt/faster_whisper_cache`) so they survive container rebuilds.
