...
- **Audio recovery remains responsive and truthful:** synchronous validation and atomic persistence run outside the Admin UI event loop, pipelines with no audio overrides return a true no-op without rotating backups, built-in profile metadata comes from the backend registry, cache invalidation is limited to actual audio-reset endpoints, success messages no longer ask operators to wait for an apply banner when no apply action is required, and a reset cannot hide or downgrade a stronger pending apply requirement.
