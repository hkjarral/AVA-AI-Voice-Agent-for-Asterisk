...
- **Full-agent provider aliases removed (branch `codex/multi-instance-full-agent-providers`)**: `AI_PROVIDER` and `contexts.*.provider` no longer accept short aliases. Replace `openai` with `openai_realtime`, `google` with `google_live`, and `deepgram_agent` with `deepgram`. Configs that still use aliases now fail validation instead of silently selecting an ambiguous provider when multiple provider instances exist.
