...
- **Provider audio contracts recover cleanly:** switching OpenAI Realtime from Beta to GA replaces stale output fields with the supported `linear16`/24 kHz pair, and legacy `mulaw`/`ulaw` 24 kHz OpenAI settings normalize to that GA contract. Invalid G.711 rate pairs identify the affected provider fields and the matching Admin UI restore action instead of leaving operators without a recovery path. Injected built-in Audio Profiles use the same canonical bodies as explicit restores.
- **Transfer handoffs fail closed on rejected Asterisk continuations** ([#577](https://github.com/hkjarral/AVA-AI-Voice-Agent-for-Asterisk/issues/577)): queue, ring-group, and extension transfers validate an explicitly missing dialplan target and require a confirmed ARI continuation before reporting success. Explicit rejection restores the previous transfer state; an indeterminate transport failure re-checks channel ownership before cleanup can reclaim a leg Asterisk may already have accepted. Dialplan discovery remains tolerant of unavailable probes so valid custom and FreePBX `ext-queues` routes can still be attempted.
- **Audio recovery remains responsive and truthful:** synchronous validation and atomic persistence run outside the Admin UI event loop, legacy shorthand pipelines return a true no-op without rotating backups, built-in profile metadata comes from the backend registry, cache invalidation is limited to actual audio-reset endpoints, and success messages no longer ask operators to wait for an apply banner when no apply action is required.
