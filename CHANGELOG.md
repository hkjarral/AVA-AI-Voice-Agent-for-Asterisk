...
- **A-law ExternalMedia codec works:** selecting `alaw` for ExternalMedia previously failed inbound decode (`Unsupported codec`) and sent PCM with a μ-law payload type. The RTP server now decodes A-law, stamps PCMA payload type 8, and the playback path resolves the companded 8 kHz leg like μ-law.
