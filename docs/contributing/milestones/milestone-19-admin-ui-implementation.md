...
Note (CLI v5.0): The public CLI surface is now `agent setup`, `agent check`, and `agent rca`. Legacy command names used below (`agent quickstart`, etc.) remain available as hidden aliases for compatibility.
