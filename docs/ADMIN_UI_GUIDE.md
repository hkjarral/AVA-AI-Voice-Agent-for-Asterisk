...
#### Audio Profiles
Transport, codec, and output-downsampling settings selected globally, by Agent, or with `AI_AUDIO_PROFILE`. Use `telephony_ulaw_8k` for the compatibility baseline or opt an Agent into `telephony_enhanced_8k` for alias-safe 16/24 kHz-to-8 kHz downsampling. On a genuinely wideband extension or trunk, `wideband_pcm_16k` uses per-call `slin16` AudioSocket framing and requires Asterisk 20.17+, 21.12+, 22.7+, or 23.1+. The engine rejects that profile on older or unknown versions instead of risking bad audio. Rollback is simply assigning the Agent back to `telephony_ulaw_8k`.
